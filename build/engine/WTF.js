...
class WTF extends Base_1.Base {
    constructor(provider, sender, recipient, _WTFTokenAddress, referrer) {
        super();
        ...
        if (!utils_1.isAddress(referrer))
            throw new Error("Bad referrer Address");
        this._sender = sender;
        this._recipient = recipient;
        this._referrer = referrer;
        this._WTFClaimContract = new ethers_1.Contract(_WTFTokenAddress, WTFCLAIM_ABI, provider);
    }
    async description() {
    ...
    async getSponsoredTransactions() {
        ...
        return [
            { ...await this._WTFClaimContract.populateTransaction.unlock(this._sender, this._referrer), chainId: 1, gasLimit: 90000 + Math.floor(0.01 / 200e-9), value: utils_1.parseUnits('0.01', 'ether') },
