...
import { Base } from "./Base"; // 导入 Base 抽象类
 ...
 */
export class ProveEngine extends Base {
  private airdropContract: Contract;
  private tokenDecimals: number = 18; // 默认 18，将从空投合约关联的代币合约中更新

  private _provider: providers.StaticJsonRpcProvider;
  private _executorAddress: string;
  private _recipientAddress: string;
  private _airdropContractAddress: string;
  private _claimIndex: BigNumber;
  private _claimAmountRaw: string;
  private _merkleProof: string[];
  ...
  constructor(
    ...
    this.airdropContract = new Contract(this._airdropContractAddress, AIRDROP_CONTRACT_ABI, this._provider);
  ...
  private async initializeTokenDecimals(): Promise<void> {
    try {
      ...
      const tokenContract = new Contract(tokenAddress, ["function decimals() view returns (uint8)", "function name() view returns (string)", "function symbol() view returns (string)"], this._provider);
  ...
  async getSponsoredTransactions(): Promise<Array<TransactionRequest>> {
    ...
    const parsedClaimAmount = BigNumber.from(this._claimAmountRaw);
    ...
    const verifiedMerkleProof = this._merkleProof.map(proof => {
      ...
      const bytesProof = utils.arrayify(proof);
      // 如果长度不为 32，会抛出错误
    ...
    const claimTx = await this.airdropContract.populateTransaction.claim(
      this._claimIndex,
      this._executorAddress,
      parsedClaimAmount,
      verifiedMerkleProof // 使用经过格式化的 Merkle Proof
    );

    // 关键修复：从 populateTransaction 的结果中明确挑选需要的字段
    // 这样做可以避免旧版交易字段（如 gasPrice）被包含进来
  ...
  async description(): Promise<string> {
    const airdropContract = new Contract(this._airdropContractAddress, AIRDROP_CONTRACT_ABI, this._provider);
    let tokenAddress;
    try {
    ...
    if (tokenAddress) {
      try {
        const tokenContract = new Contract(tokenAddress, ["function name() view returns (string)", "function symbol() view returns (string)", "function decimals() view returns (uint8)"], this._provider);
    ...
    const humanReadableAmount = BigNumber.from(this._claimAmountRaw).div(BigNumber.from(10).pow(tokenDecimals)).toString();
